        ...
        for position in entry["positions"]:
            ...
            for otherPosition in entryOutput["positions"]:
                if (
                    ...
                    and otherPosition["y"] == position["y"]
                ):
                    found = True
            if not found:
...
def ensureId(trigger):
    if "trigger" in trigger:
        if not id in trigger["trigger"]:
            ...
            trigger["trigger"][
                "id"
            ] = f"{_in['type']}/{_in['name']}/{_target['type']}/{_target['name']}"
    ...
    for overlay in overlays:
        for position in overlay["positions"]:
            ...
            if key in triggerLocations:
                ...
                overlay_type = (
                    trigger["target"]["overlay_type"]
                    if "overlay_type" in trigger["target"]
                    else None
                )
                if overlay_type == None or overlay_type == (
                    position["type"] if "type" in position else None
                ):
                    if "trigger" in trigger:
                        position["trigger"] = trigger["trigger"]
    ...
    for entry in entries:
        for position in entry["positions"]:
            ...
            if not name in result:
                result[name] = {"name": entry["name"], "score": score}
            else:
...
def addToEntries(entries, positions, key, bosses=[]):
    ...
    for entry in entries:
                    ...
                    for boss in bosses:
                        if entry["name"] == boss["from"]:
                            ...
                            if bossName in extractedBosses:
                                extractedBosses[bossName]["positions"].append(position)
                            else:
                                extractedBosses[bossName] = {
                                    "name": boss["to"],
                                    "orientation": entry["orientation"],
                                    "positions": [position],
                                }
        ...
        entry["positions"] = [
            position
            for position in entry["positions"]
            if position not in positionsToRemove
        ]
...
def cleanup(entries, layout, scenarioId, keptTokens):
    ...
    for entry in entries:
        entry["overlays"] = [
            overlay for overlay in entry["overlays"] if len(overlay["positions"]) > 0
        ]
    ...
    return [
        ...
        or len(entry["overlays"]) > 0
        or len(entry["monsters"]) > 0
    ]
...
def processMap(tileInfos, mapData, mapTriggers, scenarioSpecials):
    ...
    if len(tiles) > 0:
                    ...
                    references.append(
                        {
                            ...
                            "results": {
                                "reference": {
                                    "tile": tile["name"],
                                    "tileOrientation": f"{effectiveOrientation}",
                                }
                            },
        ...
        references.sort(
            key=lambda e: tileOrder.index(e["tile"]) if e["tile"] in tileOrder else 99
        )
                    ...
                    for reference in references:
                        ...
                        for attractor in attractors:
                            ...
                            if distance < bestDistance:
                                fuzzyDoorMatch = len(tileOrder) > 0 and (
                                    "Door" in name or key == "tokens"
                                )
        ...
        for reference in references:
            ...
            attachItemTriggersToItems(processed["overlays"], overlayTriggers)
            ...
            addToEntries(
                processed["monsters"],
                positionToMonsterLevels,
                "levels",
                list(
                    filter(
                        lambda e: e["in"] == mapData["name"] if "in" in e else True,
                        bosses,
                    )
                ),
            )
        ...
        result[
            "entries"
        ] = subResults  # cleanup(subResults, layout, scenarioId, keepTokens)
            ...
            if "maps" in scenario:
                ...
                for scenarioMap in scenario["maps"]:
                    ...
                    for removedMap in removedMaps:
                        if (
                            removedMap["type"] == scenarioMap["type"]
                            and removedMap["name"] == scenarioMap["name"]
                        ):
                            removed = True
                    if not removed:
                        mapTriggers = list(
                            filter(
                                lambda e: e["in"]["type"] == scenarioMap["type"]
                                and e["in"]["name"] == scenarioMap["name"],
                                scenarioTriggers,
                            )
                        )
                        result = processMap(
                            tileInfos, scenarioMap, mapTriggers, scenarioSpecials
                        )
                        mapsOutput.append(result)
                entriesToMap = safeGetOrEmptyList(scenarioSpecials, "entries-to-map")
                ...
                for _map in mapsOutput:
                    if "entries" in _map:
                        _map["entries"] = cleanup(
                            _map["entries"],
                            scenarioOutput["layout"]
                            if "layout" in scenarioOutput
                            else {},
                            id,
                            keepTokens,
                        )
